  ...
  if (userRoutes && userRoutes.length > 0) {
    ...
    if (layoutRoute) {
      ...
      const childRoutes = router.getRoutes().filter(route => 
        route.path.startsWith('/') && route.path !== '/' && route.path !== '/login' && route.path !== '/register'
      );
      ...
      userRoutes.forEach((route, index) => {
        const routeWithName = {
          ...route,
          name: route.name || `dynamic-route-${index}-${route.path.replace(/[/:]/g, '-')}`
        };
...
router.beforeEach(async (to, from, next) => {
  ...
  console.log('Store state:', { isLogin: store.isLogin, userInfo: store.userInfo, routes: store.routes });
